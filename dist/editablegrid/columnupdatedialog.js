...
var ColumnUpdateDialog = function (props) {
        ...
        if (column.length > 0) {
            switch (column[0].inputType) {
                case EditControlType.Date:
                    return (_jsx(DatePicker, { strings: DayPickerStrings, placeholder: "Select a date...", ariaLabel: "Select a date", className: controlClass.inputClass, onSelectDate: function (date) { return onSelectDate(date, column[0]); } }));
                ...
                case EditControlType.DropDown:
                    return (_jsx(Dropdown, { label: column[0].text, options: (_d = column[0].dropdownValues) !== null && _d !== void 0 ? _d : [], onChange: function (ev, selected) { return onDropDownChange(ev, selected, column[0]); } }));
                case EditControlType.MultilineTextField:
                    return (_jsx(TextField, { errorMessage: inputValue[column[0].key].error, className: controlClass.inputClass, multiline: true, rows: 1, placeholder: "Enter '".concat(column[0].text, "'..."), id: column[0].key, styles: textFieldStyles, onChange: function (ev, text) { return onTextUpdate(ev, text, column[0]); }, value: inputValue[column[0].key].value || '' }));
                default:
                    return (_jsx(TextField, { errorMessage: inputValue[column[0].key].error, className: controlClass.inputClass, placeholder: "Enter '".concat(column[0].text, "'..."), onChange: function (ev, text) { return onTextUpdate(ev, text, column[0]); }, styles: textFieldStyles, id: column[0].key, value: inputValue[column[0].key].value || '' }));
            }
        }
    ...
    return (_jsx(Dialog, __assign({ hidden: !inputFieldContent, onDismiss: closeDialog, closeButtonAriaLabel: "Close" }, { children: _jsxs(Stack, __assign({ grow: true, verticalAlign: "space-between", tokens: stackTokens }, { children: [_jsx(Stack.Item, __assign({ grow: 1 }, { children: _jsx(Dropdown, { placeholder: "Select the Column", options: options, styles: dropdownStyles, onChange: onSelectGridColumn }) })), _jsx(Stack.Item, __assign({ grow: 1 }, { children: GetInputFieldContent() })), _jsx(Stack.Item, { children: _jsxs(DialogFooter, __assign({ className: controlClass.inputClass }, { children: [_jsx(PrimaryButton
                            ...
                            , { 
                                // eslint-disable-next-line react/jsx-no-bind
                                onClick: saveDialog, text: "Save", disabled: (gridColumn) ? (inputValue[gridColumn].error != null && inputValue[gridColumn].error.length > 0) : false }), _jsx(DefaultButton, { onClick: closeDialog, text: "Cancel" })] })) })] })) })));
};
